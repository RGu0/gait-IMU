...
class FootSeries:
    ...
    ## 足部系是**右手系**：X 前 / Y 左 / Z 上（RAY-390 R1）

    `前 × 左 = 上`，基行列式 +1。这条不是风格问题：下游的四元数、叉乘、双足约束
    全都默认右手系，而一个左手的三元组不会让它们报错 —— **角速度是伪矢量**，改向
    的变换下它要多带一个 `det` 的符号，漏掉就只是让姿态朝反方向积，步长静默偏低而
    步频步时分毫不差。RAY-390 实测过这件事：左足步长 1.068 m 对真值 1.300 m，
    报告里没有一处看着不对。

    **Y 恒指向左，不是「外侧」。** 「外侧」在左右脚上指向相反，用它当轴的语义会
    让两只脚的手性不同 —— 那正是上面那个缺陷的来源。哪一侧是外，由 `label` 决定，
    解读归报告层。

    构造 `FootSeries` 的地方要保证送进来的量已经在这个系里。设备层的入口是
    `device/footseries.py`；那里的 module→foot 映射经真机数据核实是**恒等**。
    """
