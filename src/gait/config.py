...
class AlgoConfig:
    ...
    **下列数值未经真机标定，但也不再是纯占位。** RAY-203（零速检测）、RAY-204（ESKF）、
    ...
    #: 相邻平放段的合并间隔，s。间隔短于它的两段算同一个支撑相。
    #:
    #: 取 0.15 s 出自 T-230-04：一个支撑相被瞬时扰动（含空洞）劈成两段时不该算成
    #: 两步，而 150 ms **远短于最快档的支撑相**（实测 252~292 ms），所以它并不会
    #: 把相邻两步并掉。这是个「比最短的真支撑相小一档」的量，不是调出来的。
    stance_merge_gap_s: float = 0.15
    #: 触地撞击的回溯范围，占一个步态周期的比例。
    #:
    #: 触地是**撞击**：`|‖a‖−g|` 在那里冲到 38 m/s²，而余振落在支撑相**内部**，
    #: 所以逐样本的重力门会停在余振之后、真触地之前（实测迟到约 50 ms）。往回找那个
    #: 峰需要一个范围，而范围随速度变 —— 慢速的承重反应期本身就长。按周期取比例而
    #: 不是取固定样本数，正是为此。
    #:
    #: 取 0.30 是因为**再放大就不动了**：0.08→0.12→0.20→0.30 扫下来，平底鞋六格
    #: 从 0.12 起就不再变化，运动鞋（HOKA 厚软中底，撞击被垫子吸掉一部分）到 0.20~0.30
    #: 才饱和。饱和平台是"这是个边界不是阈值"的证据 —— 边界够宽之后答案不再依赖它。
    #: **尚未在病理步态上验证**，那里承重反应期可能更长（RAY-337）。
    stance_impact_search_fraction: float = 0.30
    #: 触地撞击的最大半高宽，s。宽于它的峰不是撞击，回溯就不采用。
    #:
    #: **撞击是瞬态，摆动相的加速度隆起是持续的** —— 这条是"回溯该不该发生"的判据，
    #: 而它必须是**形状**判据不是幅值判据：实测幅值分不开。真机触地峰 14~79 m/s²、
    #: 合成步行的摆动峰 19~58 m/s²，两个区间重叠，3 g（`anchor_threshold_m_s2`）
    #: 之类的绝对门在慢/中档漏掉 40%~97% 的真触地。半高宽则分得干干净净：
    #:
    #: | | 半高宽中位 |
    #: | --- | --- |
    #: | 真机触地（24 格） | **3~9 样本（15~45 ms）** |
    #: | 合成摆动隆起（6 例） | **15~31 样本（75~155 ms）** |
    #:
    #: 取 50 ms 落在这条缝里。个别快档步子的峰宽到 p90 ≈ 19 样本，那些步不做回溯、
    #: 保留重力门的落点 —— 退化是逐步的，不是整趟失效。
    stance_impact_max_width_s: float = 0.05
    ...
    def __post_init__(self) -> None:
        ...
        for name in (
            ...
            "stance_merge_gap_s",
            "stance_impact_search_fraction",
            "stance_impact_max_width_s",
