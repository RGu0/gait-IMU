...
def _metrics(
    ...
    一处必须知道的读法：`fraction` 走的是**产品口径**（`analysis/events.py`），
    而那个口径在大偏差下会因相位配对错乱而跳变（见 `paired_leading_difference`
    的文档）。跳变不是本管线的缺陷，它就是用户在那种同步质量下真会看到的读数 ——
    所以照报，由配对口径提供不受错乱影响的物理参照。
    """
...
def paired_double_support(
    ...
    那两处都按「区间起点排序后取相邻对」配相位，同足相邻就跳过。对它们各自的
    输入那是对的，但本模块要做的事不一样：**同一份数据要在平移过的时间轴上再算
    一遍**。平移量一大，排序后的交替序列就会出现同足相邻，被跳过的相位不是随机
    的几个，于是两类均值之差整个跳掉。

    实测（合成对称步态，细化后的生理边界）：Δ=10/20 ms 时相邻法给出 +20.00/
    +40.00 ms，与 2Δ 分毫不差；Δ=30 ms 时给出 **−128.87 ms** —— 而 PRD §8 预期的
    误差上界正是 30 ms，也就是说失效恰好落在被测范围里。

    这里改用**区间交集**：双支撑相位 = 两足支撑区间的重叠 `min(to) − max(ic)`，
    先离地的一足 = `to` 较小的那只。它不依赖任何排序相邻性，因此在平移下连续 ——
    分类只在两足离地顺序**真正翻转**时才变，而那时它本来就该变。
    """
        ...
        metrics=(
            ...
            MetricBias(
                ...
                if comparable
                    ...
                    "两次读数不是同一个量，相减无意义。多见于两足事件恰好落在同一"
                    "采样网格上的数据（合成步态尤甚）；真机上两足各有时间轴，罕见。"
                ),
            ),
            MetricBias(
