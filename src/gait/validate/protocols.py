...
def course_length_from_pitch(pitch_m: float, placements_per_foot: int) -> float:
    ...
    T-230-03 的走廊就是这样量出来的：每足 38 次落脚 ⇒ `2×38 − 1` = **75** 个单步，
    而现场逐块数出来的砖块数正是 75。两个计数来源无关却对上了 —— 这是「并脚起步、
    并脚收尾」这个前提唯一的独立检验，也是它成立的证据。

    **注意 `pitch_m` 要用平均砖距。** 该走廊实测**不均匀**（前 40 块 600.90 mm、
    后 35 块 601.914 mm），所以由局部砖距外推全程会错 35.5 mm。全程长度请直接量，
    本函数用于「已知平均单步进距，反推走廊长」这一向。
...
def stride_length_from_placements(distance_m: float, placements_per_foot: int) -> float:
    ...
    * 正确：`45.103 / 37.5` = **1.2028 m**（= 两个平均砖距，与场地自洽）
    * 错误：`45.103 / 38` = 1.1869 m —— **低 1.3%**
...
class TrialGeometry:
    ...
    #:
    #: * `straight`：整趟直线长度（如 T-230-03 走廊全程实测的 45.103 m）。读数是首末位移模长。距离**由本
