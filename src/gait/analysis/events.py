...
## 五、`double_support()` 的输入必须是**已分段**的步

这一条在 RAY-211 交付时只存在于测试的辅助函数里（`tests/test_events.py` 的
`dual_cycles()` 先调 `drop_still_lead()` 再 `segment_cycles()`），模块本身一个字
没说。RAY-290 把它补上，因为已经有人踩中过。

`double_support()` 按「起点排序后取相邻对」配相位，对每一对都当作一个双支撑期。
起步前的静止前导会被 ZUPT 检成一个**很长的支撑相**（实测 1.67~2.23 s，而典型双
支撑期只有 110 ms）。它一旦进来，就会以一个秒级的"相位"混进均值：实测占比从
20.5% 读成 30.5%，**污染 7~10 个百分点**，而生理带宽本身才 10~25%。

产品路径是干净的：`cloud/chain.py` 传进来的是 `feet[...].selected`，即
`analysis/segments.py` 选出的中段直行步 —— 前导既不是直行段也不在中段，早就没了。
**没有走分段就直接喂 `segment_cycles()` 输出的调用方需要自己剔前导。**
`cli/v3prime.py` 的 V3′ 实验工装正是这样一个调用方 —— 它此前没剔，读数因此被污染；
RAY-296 已让它在 ZUPT 边界上剔（**在细化之前**：细化把典型支撑相拉长的比例比前导大，
比值 2.86 → 2.49，同一条判据在细化后就不再认得出前导）。

本函数不自己剔，是因为"哪些步算数"是分段的职责（`segments.py` 的 `trim` 是可回溯
参数，FR-312 要求可复查）。在这里再剔一次会出现两处互不知情的剔除策略。
