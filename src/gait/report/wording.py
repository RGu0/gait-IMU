...
#: 专业参数区的那一句：双支撑期读数与**生理**双支撑期的系统性差异（RAY-288 范围 1）。
#:
#: 核心指标卡上那个标注只够说「这是哪个口径」，说不下**差多少、为什么差**。这句话
#: 补的正是那一半 —— PRD §8 把双支撑期定为「观测量记录」而不是判据，靠的就是这个
#: 差异；报告上不写，读者会拿它跟文献里的生理双支撑期直接比。
#:
#: 措辞受 PRD §12 约束：不用诊断措辞，不说「偏低了所以有问题」—— 它不是异常，
#: 是口径差。
DOUBLE_SUPPORT_EXPLAINER: Final[str] = (
    "双支撑期占比由零速（ZUPT）边界算出，"
    "而零速边界比生理上的触地／离地时刻窄：触地与离地的过渡段都不算在内，"
    "单侧约 50 ms，因此该读数系统性地比生理双支撑期低约 100 ms —— "
    "正常快走时读数为负属于此，不是异常。"
    "与文献中的生理双支撑期不可直接比较。"
    "（削减量出自合成探针，尚未用测力台或压力垫在真机上标定。）"
)
