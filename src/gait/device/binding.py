...
class FootBinding:
    ...
    不可变：每个操作返回新的实例。就地修改会让「这次会话用的是哪套绑定」变得
    取决于读取时机 —— 而那正是事后追溯要问的问题。

    ⚠️ **那条追溯链今天还没有接上**（RAY-479）。本行此前写的是「绑定关系会被写进
    会话元数据用于事后追溯」，**那句不成立**：本模块在生产代码里零调用，绑定既不
    落盘也不进会话元数据。`app/service.py::_device_records()` 写进
    `SessionMeta.devices` 的是**采集层读到的设备身份**，不是绑定 —— 两者在正常情况
    下指向同一台设备，但回答的不是同一个问题，而绑反时恰好会分叉。

    所以不可变性目前是**为接线做好的准备**，不是一个正在生效的保证。RAY-479 落地
    之前不要把这段改回「已经在用」。
    """
