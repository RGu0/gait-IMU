...
（`S1-sport/fast-b`）上量了四条路，**没有一条能让上限门变红**：

| 注入 | 结果 |
| -- | -- |
| 恒定陀螺 z 零偏 0 → 16 °/s | 中位 8.8 → 16.5°，**且非单调**（8.1 / 5.6 / 7.0 / 9.1） |
| 斜坡零偏 0 → 40 °/s（模拟温漂） | 中位 17.4°，仍在带内 |
| GLRT 阈调高（观测变**密**） | 中位 **降**到 7.0°，ZUPT 16.4% → 27.2% |
| GLRT 阈调低（饿死观测） | 初始对准先抛 `AlignmentError`，漂移没升起来 |

**前两条本身就是结论**：ESKF 的零偏状态把恒定零偏和慢斜坡都吸收掉了 —— 那正是
ZARU 该干的事。它独立复证了 RAY-356 第六轮的「陀螺零偏残余不是根因」，这次是从
注入侧证的。第三条复证了判据 4 的机制方向在**干预**下也成立，而不只是相关。

第四条说明上限门为什么测不了：要让中位越过 40°，每一步都得像转身（阈 25°），
而那必然先杀死分段或对准。**所以上限门是一道没有阳性对照的"不得更坏"门，这里明写
出来，而不是假装它通了电。**
