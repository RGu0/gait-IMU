...
上面的边界一节冻结在 RAY-350 那次审计的时点。**冻结的边界比会腐烂的边界诚实**，
但一张不记录后续变化的图会慢慢变成谎话。所以这里只追加**新落地的守卫**，
不重做审计 —— 重做要另立 Issue。

| Issue | 判据 | 守卫 | 类 |
| --- | --- | --- | --- |
| RAY-351 | 判据 1、2：产品链路印在报告上的双支撑期占比、支撑相占比 | `chain_metrics.py` | 真机 |
| RAY-356 | 判据 1~4：航向漂移带、转身误报带、速度依赖、机制相关 | `heading_drift.py` | 真机 |

**RAY-351 在上面的边界表里被列为「未交付的 Backlog」，它此后交付了。** 这行本身
就是本节存在的理由：边界表是快照，快照会过期，而过期的快照不会自己说出来。

### RAY-356 的上限门没有阳性对照 —— 明记为已知限制

`heading_drift.py` 的判据 1 是双向带 `[2, 40]°`。**下限门有阳性对照**（合成步态
实测 0.07~0.21°，触红），**上限门没有**：中位随注入的陀螺零偏成比例爬到 **22.8°**
仍在带内，再往上加就抛 `SegmentationError` —— 每一步都像转身（阈 25°）时，
`separate` 已经找不出直行段。**分段死在中位越线之前，中间没有窗口。**
逐条读数见该脚本的模块头，那里也记了两个被同一批实验推翻的初步说法。

按本图「守法四类」，上限门既不是「无」（它会跑、会判），也不同于其余有对照的门。
**这里不给它新造一类，而是把限制写在这，免得下一个人以为它验过。**
