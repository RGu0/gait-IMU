...
| RAY-362 | 判据 2~4：上限门的阳性对照、阴性对照、三种结局分开报 | `heading_drift.py` | 真机 |

**RAY-351 在上面的边界表里被列为「未交付的 Backlog」，它此后交付了。** 这行本身
就是本节存在的理由：边界表是快照，快照会过期，而过期的快照不会自己说出来。

### 两道门现在都有阳性对照（RAY-362 更正了 RAY-356 的记载）

`heading_drift.py` 的判据 1 是双向带 `[2, 40]°`，**两道门各有对照**：

| 门 | 对照 | 实测 |
| --- | --- | --- |
| 下限 2° | 合成步态走同一条链 | 0.07~0.21°，触红 |
| 上限 40° | `S1-sport/slow-b` 注入斜坡零偏 50 °/s（起点 1.5 s） | 左脚 61.2°，触红 |
| —— | **阴性对照**：同一格 20 °/s，必须留在带内 | 24.0 / 18.6° |

**本节此前写的是「上限门没有阳性对照」，那是错的。** 那个结论出自两条**一维**扫描
（各自固定 onset 扫峰值），两条线都撞上 `SegmentationError`，于是各自读成「分段死在
中位越线之前，中间没有窗口」。补上 peak × onset 的二维网格才看清：**快档确实散
（红 9/54、崩 39/54），慢档不散（红 39/54、崩 1）** —— 散的是快档，不是这条路。

**这条更正本身属于本图要防的那一类，而且是反向的。** 上面「审计边界」一节说过，
一张不说自己边界的覆盖图会被当成「全都查过了」；而写下一条**不成立的已知限制**
更糟 —— 它让下一个人以为这个对照**验不了**，因而根本不会去试。
**「验过」与「验不了」都要能被推翻；写下限制不等于免除复核。**

顺带一条给本图所有「无人守」条目的提醒：它们里可能也有这一类 ——
**记录说没人守，不等于守不住。**

## 双支撑期为负有**两种**成因，只有一种归分段（RAY-354 判据 4）

RAY-354 的溯因轮把 DS 变负拆成了两类，**它们的修法在不同的 Issue 上**：

| 模式 | 症状 | 成因 | 归属 |
| --- | --- | --- | --- |
| **A** | 中位为**正**（+0.03~+0.09），占比被少数极端负相位毁掉 | 分段碎片化 → 两足中段步集合对不上 → 配对跨过缺步 | **RAY-354**（已修：判据 6 + 7） |
| **B** | 中位为**负**，整条分布下移 | 支撑相区间检出太短，两足之间出现本不该有的空隙 | **RAY-325 线**，不归 RAY-354 |

**判别量是支撑相占比，不是转身数**：

| 相关 | r |
| --- | --- |
| **支撑相占比 vs DS 中位** | **+0.957** |
| 转身总数 vs DS 中位 | +0.457 |

模式 B 的支撑相占比 **39.5 ~ 48.5%**，模式 A 是 **51.3 ~ 53.5%**（生理约 60%）。

**为什么要把它们分开写下来。** RAY-354 判据 6（跨足一致性）落地后，模式 A 的四个
慢档格全部转正，而模式 B 的 `S1-sport/fast-a` / `fast-b` **读数一位不差**
（−0.2158 / −0.2215）。**混成一个判据，会让治好其中一条的人看不到进展** ——
那正是 RAY-351 之前「稳定地错、对好坏不敏感」的翻版，而这张图存在的理由就是
让这种事看得见。

**所以 `chain_metrics.py` 的 DS 门只对模式 A 负责。** 模式 B 由 `stance_intervals.py`
的支撑相占比带（RAY-325 / RAY-346）守着，两者不要互相顶替。
