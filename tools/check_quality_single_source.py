...
第 3 条另有两处已知的够不着：非十进制字面量（`n === 0x10`）与把数字包进括号
（`n === (0)`）。两者都是逐行正则的边界，不是取舍 —— 但也都不是会顺手写出来的
形状，所以记在这里而不是继续往正则里堆。

三条规则都是**逐行**匹配，没有词法分析：字符串里出现 `"n === 0"`、或行尾注释里
写了同样的比较，同样会被拦。这一点第 2 条本来就是如此（实测同形），第 3 条没有
把它变好也没有变坏。
...
_EQUALITY = r"[=!]==?"
#: 右侧允许一个负号：`n === -1` 这种哨兵值是现实写法。左侧不需要 —— `-1 === n`
#: 里的 `1` 前面是 `-`，本来就过得了 `_NUMBER` 的左界。
